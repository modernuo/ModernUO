...
- **Convention sweep — DONE 2026-06-07:** audited all touched `.cs` against CLAUDE.md. Result was largely clean (no Tier-3 LINQ on craft paths, no `Console`, no concurrency primitives, correct serialization of v5 jewel + transient context fields, `stackalloc`-based name formatting, braces throughout). **Fixed:** (I-1) `new List<Item>()` → `PooledRefList<Item>.Create()` in `CraftItem.ConsumeHuedAmount` (warm path); (I-2) removed a dead discarded `GetContext` call in `Repair.cs`; (I-3) removed a commented-out line in `CraftItem.cs`. **Deliberately not changed (faithful to Jack, low impact):** `Activator.CreateInstance` in `T2ACraftSystem.ShowMenuDirect<T>` (runs once per tool double-click, not per-craft — could later be a compiled factory), an empty `OnCancel` override in `BlacksmithMenu`, and minor blank-line style. The `InscriptionMenu._spellIds`/`_args` static caches are bounded and acceptable on a single-threaded server.
