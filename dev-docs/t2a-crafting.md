...
The targeted hue is carried into the **product only when the resource actually retains color** — i.e. when `CraftItem.RetainsColorFrom(system, resource)` is true (`ConsumeHuedRes` gates the output `resHue` on it). Crucially, **leather/hides only retain color in AOS+** (`CraftItem.RetainsColorFrom` returns false for `BaseLeather`/`BaseHides` when `!Core.AOS`), so a pre-AOS leather item is always its default (uncolored) appearance even if crafted from dyed leather. Colored **ingots/ore** still color metal armor/weapons/shields pre-AOS via the standard path.

> **Coloring rule (general):** an item takes the consumed resource's hue when `RetainsColorFrom` is true = the craft system's own override OR (item ∈ {BaseWeapon, BaseArmor, BaseClothing, BaseJewel, DragonBardingDeed} AND resource ∈ {BaseIngot, BaseOre, BaseLeather*, BaseHides*, Cloth, UncutCloth, BaseGranite, BaseScales}). *Leather/hides gated to AOS+. Wood (`BaseLog`/`BaseBoard`) is not in the table, so carpentry never colors.
>
> **Deferred (pending research):** Stratics (2001) indicates **weapons** were *not* colored pre-Publish-14 even though the resource was conveyed (e.g. "a valorite viking sword" rendered iron-colored). When confirmed, this likely needs a `BaseWeapon`-specific change that still sets `Resource` but suppresses hue for `!Core.UOTD`. Not yet implemented.
