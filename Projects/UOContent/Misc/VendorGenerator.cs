    ...
    public static class VendorGenerator
    {
        ...
        private static void Process(Map map, Rectangle2D[] regions)
        {
            ...
            for (var i = 0; i < m_ShopList.Count; ++i)
            {
                ...
                for (var j = 0; j < si.m_Floor.Count; ++j)
                {
                    var fp = si.m_Floor[j];
                    ...
                    var eable = map.GetItemsInRange<Spawner>(new Point3D(fp.X, fp.Y, 0), 0);
        ...
        {
            var flags = tiles.Aggregate(ShopFlags.None, (current, t) => current | ProcessDisplayedItem(t.ID));

            if (flags != ShopFlags.None)
            {
                var p = new Point2D(x, y);

                if (m_ShopTable.TryGetValue(p, out var si))
                {
                    si.m_Flags |= flags;
                }
                else
