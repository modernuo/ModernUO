...
public static class ClientVerification
{
    ...
    private static void EventSink_ClientVersionReceived(NetState state, ClientVersion version)
    {
        ...
        else
        {
            ...
            {
                message.Append("This server does not allow classic clients to connect.");
                shouldKick = true;
            }
            ...
            {
                message.Append("This server does not allow UO:KR clients to connect.");
                shouldKick = true;
            }
            else if (!AllowSA && version.Type == ClientType.SA)
            {
                message.Append("This server does not allow UO:SA clients to connect.");
                shouldKick = true;
            }

            if (message.Length > 0)
            {
                message.Append(_allowedClientsMessage);
            }
        }
