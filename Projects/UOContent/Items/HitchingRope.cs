...
public partial class HitchingRope : Item
{
    [Constructible]
    public HitchingRope()
        : base( 0x14F8 )
    ...
    public override void OnDoubleClick( Mobile from )
    {
        if ( IsChildOf( from.Backpack ) || from.InRange( GetWorldLocation(), 2 ) && Movable )
        {
            from.LocalOverheadMessage(
                ...
                0x3B2,
                1071159
            ); // Select the hitching post you want to supply hitching rope.
    ...
    private class InternalTarget : Target
    {
        ...
        protected override void OnTarget( Mobile from, object targeted )
        {
            ...
            {
                var postItem = ( HitchingPost )targeted;

                if ( postItem.UsesRemaining >= 1 )
                {
                    from.SendMessage( 0x59, "Hitching Rope cannot be applied at this time." );
                }
