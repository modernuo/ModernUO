...
public partial class CrystalPortal : Item, ISecurable
{
    [Constructible]
    ...
    public override void OnSpeech( SpeechEventArgs e )
    {
        ...
        if ( e.Mobile.Murderer && map != Map.Felucca )
        {
        ...
        {
            if ( SpellHelper.CheckTravel( e.Mobile, map, loc, TravelCheckType.RecallTo, out var message ) )
            {
                OnTeleport( e.Mobile, loc, map );
            }
            else
            {
                e.Mobile.LocalOverheadMessage( MessageType.Regular, 0x4F1, message ); // You cannot teleport into that area.
            }
        }
    }
