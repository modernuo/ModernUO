...
namespace Server.Multis
{
    public abstract class BaseHouse : BaseMulti
    {
        ...
        public virtual bool CheckAosStorage(int need) =>
            GetAosCurSecures(out int fromSecures, out int fromVendors, out int fromLockdowns, out int fromMovingCrate) +
            need <=
            GetAosMaxSecures();
            ...
            else if (to.Player)
            {
                ...
                else if (CheckTransferPosition(from, to))
                {
                    ...
                    if (HasRentedVendors)
                    {
                         ...
                         */
                        to.SendGump(
                            new WarningGump(1060635, 30720, 1062487, 32512, 420, 280,
