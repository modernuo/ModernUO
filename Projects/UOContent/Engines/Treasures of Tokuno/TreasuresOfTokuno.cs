...
namespace Server.Gumps
{
    ...
    public class ToTTurnInGump : BaseImageTileButtonsGump
    {
        ...
        public ToTTurnInGump(Mobile collector, List<ItemTileButtonInfo> buttons) : base(1071012,
            Utility.CastListContravariant<ItemTileButtonInfo, ImageTileButtonInfo>(
                buttons)) // Click a minor artifact to give it to Ihara Soko.
            =>
    ...
    public class ToTRedeemGump : BaseImageTileButtonsGump
    {
        ...
        public override void HandleButtonResponse(NetState sender, int adjustedButton, ImageTileButtonInfo buttonInfo)
        {
            ...
            {
                pm.ToTItemsTurnedIn -= TreasuresOfTokuno.ItemsPerReward;
                m_Collector.SayTo(pm, 1070984,
                    item.Name == null || item.Name.Length <= 0
                        ? $"#{item.LabelNumber}"
                        : item.Name); // You have earned the gratitude of the Empire. I have placed the ~1_OBJTYPE~ in your backpack.
            }
            else
            {
                item.Delete();
        ...
        {
            public TypeTileButtonInfo(Type type, int itemID, TextDefinition label, int localizedToolTip = -1) : this(type,
                itemID,
                0, label, localizedToolTip)
            {
            }

            public TypeTileButtonInfo(Type type, int itemID, int hue, TextDefinition label, int localizedToolTip = -1) :
                base(
