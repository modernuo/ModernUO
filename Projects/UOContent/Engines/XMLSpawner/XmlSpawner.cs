...
public class XmlSpawner : Item, ISpawner
{
    ...
    private const bool SectorIsActive = false;
        ...
        get
        {
            ...
            foreach (var so in m_SpawnObjects)
            {
                for (var x = 0; x < so.SpawnedObjects.Count; x++)
                {
                    ...
                    {
                        var bsec = creature.Map.GetSector(creature.Location);
                ...
                for (var x = x1; x <= x1 + width; x += Map.SectorSize)
                {
                    for (var y = y1; y <= y1 + height; y += Map.SectorSize)
                    {
                        ...
                        if (!duplicate)
                        {
                            ...
                            if (GlobalSectorTable[Map.MapID].TryGetValue(s, out var spawnerlist)) //.Contains(s))
                            {
    ...
    public bool OnHold
    {
        get
        {
            ...
            foreach (var sot in m_KeywordTagList)
            {
                ...
                if (sot?.Deleted == false && (sot.Flags & BaseXmlSpawner.KeywordFlags.HoldSpawn) != 0)
                {
                    return true;
                }
            }
    ...
    public bool HoldSequence
    {
        get
        {
            ...
            foreach (var sot in m_KeywordTagList)
            {
                ...
                if (sot?.Deleted == false && (sot.Flags & BaseXmlSpawner.KeywordFlags.HoldSequence) != 0)
                {
                    return true;
                }
            }
    ...
    public string RegionName
    {
        ...
        set
        {
            ...
            {
                if (region.Name.InsensitiveEquals(m_RegionName))
                {
                    m_Region = region;
    ...
    [CommandProperty(AccessLevel.GameMaster)]
    public bool HomeRangeIsRelative { get; set; }
    ...
    public bool TODInRange
    {
        get
        {
            ...
            var TOD_start = day_start + TODStart;
            var TOD_end = day_start + TODEnd;
    ...
    public bool SmartSpawning
    {
        ...
        set
        {
            ...
            if (m_SmartSpawning)
            {
                ...
                if (m_GlobalSectorTimer?.Running != true)
                {
    ...
    {
        if (from?.Deleted != false || from.AccessLevel < AccessLevel.GameMaster || SpawnerGump != null && SomeOneHasGumpOpen)
        {
            return;
        }
    ...
    public override void OnDelete()
    {
        ...
        if (m_ShowContainerStatic?.Deleted == false)
        {
    ...
    public static int ConvertToInt(string value) =>
        value.StartsWith("0x") ? Convert.ToInt32(value.Substring(2), 16) : Convert.ToInt32(value);
        ...
        else
        {
            ...
            try
            {
                ...
                if (o is Mobile mobile)
                {
                    ...
                    _ = BaseXmlSpawner.ApplyObjectStringProperties(null, substitutedtypeName, mobile, trigmob, attachedto, out _);
                }
    ...
    private static void RemoveFromSectorTable(Sector s, XmlSpawner spawner)
    {
        ...
        // find the sector
        if (GlobalSectorTable[s.Owner.MapID].TryGetValue(s, out var spawnerlist) && spawnerlist != null)
        {
            //List<XmlSpawner> spawnerlist = GlobalSectorTable[s.Owner.MapID][s];
            if (spawnerlist.Contains(spawner))
            {
                _ = spawnerlist.Remove(spawner);
            }
        }
    }
    ...
    public void LoadXmlConfig(string filename)
    {
        ...
        if (File.Exists(filename))
        {
            ...
            {
                if (ds.Tables[XmlTableName]?.Rows.Count > 0)
                {
                    foreach (DataRow dr in ds.Tables[XmlTableName].Rows)
                    {
                        ...
                        if (valid_entry) { DespawnTime = TimeSpan.FromHours(doubleEntry); }

                        valid_entry = true;
                        try { doubleEntry = double.Parse((string)dr["MinRefractory"]); }
                        catch { valid_entry = false; }
                        if (valid_entry) { RefractMin = TimeSpan.FromMinutes(doubleEntry); }

                        valid_entry = true;
                        try { doubleEntry = double.Parse((string)dr["MaxRefractory"]); }
                        catch { valid_entry = false; }
                        if (valid_entry) { RefractMax = TimeSpan.FromMinutes(doubleEntry); }

                        valid_entry = true;
                        try { doubleEntry = double.Parse((string)dr["TODStart"]); }
                        catch { valid_entry = false; }
                        if (valid_entry) { TODStart = TimeSpan.FromMinutes(doubleEntry); }

                        valid_entry = true;
                        try { doubleEntry = double.Parse((string)dr["TODEnd"]); }
                        catch { valid_entry = false; }
                        if (valid_entry) { TODEnd = TimeSpan.FromMinutes(doubleEntry); }
    ...
    private void ActivateTrigger()
    {
        ...
        {
            var minSeconds = (int)RefractMin.TotalSeconds;
            var maxSeconds = (int)RefractMax.TotalSeconds;
    ...
    public void CheckTriggers(Mobile m, Skill s, bool hasproximity)
    {
        if (AllowTriggering && !m_proximityActivated) // only proximity trigger when no spawns have already been triggered
        {
            ...
            if (Utility.RandomDouble() < TriggerProbability)
            {
                ...
                if (ProximitySound > 0 && m?.Deleted == false)
                {
                    m.PlaySound(ProximitySound);
                }

                // display the trigger message
                if (!string.IsNullOrEmpty(ProximityMsg) && m?.Deleted == false)
                {
    ...
    public bool HandlesOnSkillUse => m_Running && SkillTrigger?.Length > 0;
    ...
    public void AddToMovementList(Mobile m)
    {
        ...
        if (m_MovementTimer?.Running != true)
        {
    ...
    public static bool AssignSettings(string argname, string value)
    {
        switch (argname)
        {
            ...
            case "DiskAccessLevel":
                {
                    DiskAccessLevel = Enum.Parse<AccessLevel>(value, true);
                    break;
                }
            case "SmartSpawnAccessLevel":
                {
                    SmartSpawnAccessLevel = Enum.Parse<AccessLevel>(value, true);
                    break;
                }
            ...
            case "ChangeCommand":
                {
                    ...
                    _ = Timer.DelayCall(TimeSpan.Zero, DelayedAssignSettings, argname, value);
                    break;
                }
            default:
                {
    ...
    public static void ShowTagList(XmlSpawner spawner)
    {
        var count = 0;
    ...
    private static void LoadDefaults(XmlElement node)
    {
        ...
        defTODMode = todmode switch
        {
            (int)TODModeType.Realtime => TODModeType.Realtime,
            (int)TODModeType.Gametime => TODModeType.Gametime,
            _                         => defTODMode
        };
    }
        ...
        if (e.Arguments.Length >= 1)
        {
            ...
            if (e.Arguments.Length == 2)
            {
                ...
                else if (e.Arguments[0].ToLower() == "todmode")
                {
                    try
                    {
                        var todmode = Convert.ToInt32(e.Arguments[1]);
                        defTODMode = todmode switch
                        {
                            (int)TODModeType.Gametime => TODModeType.Gametime,
                            (int)TODModeType.Realtime => TODModeType.Realtime,
    ...
    public static void ShowSpawnPoints_OnCommand(CommandEventArgs e)
    {
        ...
        foreach (var xml_item in ToShow)
        {
            ...
            if ((xml_item.m_ShowContainerStatic == null || xml_item.m_ShowContainerStatic.Deleted) && xml_item.RootParent is Container rootItem)
            {
                ...
                var x = rootItem.Location.X;
                var y = rootItem.Location.Y;
                var z = rootItem.Location.Z + 10;
    ...
    public static void HideSpawnPoints_OnCommand(CommandEventArgs e)
    {
        ...
        foreach (var item in World.Items.Values)
        {
            if (item is XmlSpawner xmlItem)
            {
                ...
                if (xmlItem.m_ShowContainerStatic?.Deleted == false)
                {
        ...
        {
            if (xml_item.m_ShowContainerStatic?.Deleted == false)
            {
    ...
    private static void SpawnEditorGo_OnCommand(CommandEventArgs e)
    {
        ...
        if (from != null && e.Length >= 1)
        {
            ...
            Map NewMap;
            // Convert the xml map value to a real map object
            if (MapName.InsensitiveEquals(Map.Trammel.Name))
            {
                NewMap = Map.Trammel;
            }
            else if (MapName.InsensitiveEquals(Map.Felucca.Name))
            {
                NewMap = Map.Felucca;
            }
            else if (MapName.InsensitiveEquals(Map.Ilshenar.Name))
            {
                NewMap = Map.Ilshenar;
            }
            else if (MapName.InsensitiveEquals(Map.Malas.Name))
            {
                NewMap = Map.Malas;
            }
            else if (MapName.InsensitiveEquals(Map.Tokuno.Name))
            {
                NewMap = Map.Tokuno;
            }
            else
    ...
    public static void XmlUnLoadFromFile(string filename, string SpawnerPrefix, Mobile from, out int processedmaps, out int processedspawners)
    {
        ...
        else if (Directory.Exists(filename)) // check to see if it is a directory
        {
    ...
    public static void XmlUnLoadFromStream(Stream fs, string filename, string SpawnerPrefix, Mobile from, out int processedmaps, out int processedspawners)
    {
        ...
        if (ds.Tables.Count > 0)
        {
            ...
            if (ds.Tables[SpawnTablePointName]?.Rows.Count > 0)
            {
                foreach (DataRow dr in ds.Tables[SpawnTablePointName].Rows)
                {
                    ...
                    if (SpawnerPrefix == null || SpawnerPrefix.Length == 0 || SpawnName.StartsWith(SpawnerPrefix))
                    {
                        ...
                        // Convert the xml map value to a real map object
                        if (XmlMapName.InsensitiveEquals(Map.Trammel.Name) || XmlMapName == "Trammel")
                        {
                            SpawnMap = Map.Trammel;
                            TrammelCount++;
                        }
                        else if (XmlMapName.InsensitiveEquals(Map.Felucca.Name) || XmlMapName == "Felucca")
                        {
                            SpawnMap = Map.Felucca;
                            FeluccaCount++;
                        }
                        else if (XmlMapName.InsensitiveEquals(Map.Ilshenar.Name) || XmlMapName == "Ilshenar")
                        {
                            SpawnMap = Map.Ilshenar;
                            IlshenarCount++;
                        }
                        else if (XmlMapName.InsensitiveEquals(Map.Malas.Name) || XmlMapName == "Malas")
                        {
                            SpawnMap = Map.Malas;
                            MalasCount++;
                        }
                        else if (XmlMapName.InsensitiveEquals(Map.Tokuno.Name) || XmlMapName == "Tokuno")
                        {
                            SpawnMap = Map.Tokuno;
                            TokunoCount++;
                        }
                        ...
                        foreach (var i in World.Items.Values)
                        {
                            if (i is XmlSpawner checkXmlSpawner)
                            {
                                ...
                                {
                                    if (checkXmlSpawner != null)
                                    {
                                        spawners_deleted++;
                                        checkXmlSpawner.Delete();
                                    }
    ...
    public static void XmlImportMap(string filename, Mobile from, out int processedmaps, out int processedspawners)
    {
        ...
        var total_processed_maps = 0;
        var total_processed_spawners = 0;
        if (filename == null || filename.Length <= 0 || from?.Deleted != false)
        {
            return;
        }
            ...
            if (!badspawn && totalspawns > 0)
            {
                ...
                for (var k = 0; k < 6; k++)
                {
                    ...
                    for (var i = 0; i < typenames[k].Length; i++)
                    {
                        ...
                        var type = AssemblyHandler.FindTypeByName(typenames[k][i]);
                ...
                var spawnername = $"{Path.GetFileNameWithoutExtension(filename)}#{spawnercount}";
    ...
    private static void ParseOldMapFormat(Mobile from, string filename, string line, string[] args, int linenumber, ref int spawnercount, ref int badspawnercount, ref int overridemap, ref double overridemintime, ref double overridemaxtime)
    {
        ...
        if (args.Length > 0 && args[0] == "*")
        {
            ...
            if (args.Length is not 11 and not 12)
            {
                badspawn = true;
            ...
            if (!badspawn && typenames.Length > 0)
            {
                ...
                for (var i = 0; i < typenames.Length; i++)
                {
                    ...
                    // check the type to see if there are vendors on it
                    var type = AssemblyHandler.FindTypeByName(typenames[i]);
    ...
    private static void ImportSpawner(XmlElement node)
    {
        ...
        for (var i = 0; i < creaturesName.Count; i++)
        {
            ...
            // check the type to see if there are vendors on it
            var type = AssemblyHandler.FindTypeByName(creaturesName[i]);
        ...
        if (ds.Tables.Count > 0)
        {
                ...
                foreach (DataRow dr in ds.Tables[SpawnTablePointName].Rows)
                {
                    ...
                    if (string.IsNullOrEmpty(SpawnerPrefix) || SpawnName.StartsWith(SpawnerPrefix))
                    {
                        ...
                        if (!bad_spawner)
                        {
                            ...
                            var TheSpawn = new XmlSpawner(SpawnId, SpawnX, SpawnY, SpawnWidth, SpawnHeight, SpawnName, SpawnMaxCount,
                            ...
                            if (found_container && spawn_container?.Deleted == false)
                            {
            ...
            if (ds.Tables[SpawnTablePointName]?.Rows.Count > 0)
            {
                foreach (DataRow dr in ds.Tables[SpawnTablePointName].Rows)
                {
                    ...
                    if (found_spawner && OldSpawner?.Deleted == false)
                    {
                        ...
                        if (!string.IsNullOrEmpty(setObjectName))
                        {
                            ...
                            if (loadnew)
                            {
                                var tmpsetObjectName = $"{namestr}-{newloadid}";
                        ...
                        if (!string.IsNullOrEmpty(triggerObjectName))
                        {
                            ...
                            if (loadnew)
                            {
                                var tmptriggerObjectName = $"{namestr}-{newloadid}";
    ...
    public class XmlSaveSingle : BaseCommand
    {
        ...
        public override void Execute(CommandEventArgs e, object obj)
        {
            if (e?.Mobile == null || e.Arguments == null)
            {
                return;
            }
            ...
            if (obj is not XmlSpawner xmlspawner)
            {
    ...
    private static void SaveSpawns(Mobile m, XmlSpawner xmlspawner, string filename)
    {
        ...
        if (Directory.Exists(XmlSpawnDir) && filename?.StartsWith("/") == false && !filename.StartsWith("\\"))
        {
    ...
    {
        if (e?.Mobile == null || e.Arguments == null || e.Arguments.Length < 1)
        {
            return;
        }
        ...
        foreach (var i in World.Items.Values)
        {
            if (i is XmlSpawner spawner && !spawner.Deleted && (SaveAllMaps || spawner.Map == e.Mobile.Map)
                ...
                && spawner.RootParent is not Mobile
                && (SpawnerPrefix == null || SpawnerPrefix.Length == 0 || spawner.Name?.StartsWith(SpawnerPrefix) == true))
            {
    ...
    public static bool SaveSpawnList(Mobile from, List<XmlSpawner> savelist, string dirname, Stream stream, bool oldformat, bool verbose)
    {
        ...
        foreach (var sp in savelist)
        {
            ...
            // Convert the xml map value to a real map object
            if (sp.Map.Name.InsensitiveEquals(Map.Trammel.Name))
            {
                TrammelCount++;
            }
            else if (sp.Map.Name.InsensitiveEquals(Map.Felucca.Name))
            {
                FeluccaCount++;
            }
            else if (sp.Map.Name.InsensitiveEquals(Map.Ilshenar.Name))
            {
                IlshenarCount++;
            }
            else if (sp.Map.Name.InsensitiveEquals(Map.Malas.Name))
            {
                MalasCount++;
            }
            else if (sp.Map.Name.InsensitiveEquals(Map.Tokuno.Name))
            {
                TokunoCount++;
            }
            else
            ...
            if (sp.m_ObjectPropertyItem?.Deleted == false)
            {
            ...
            if (sp.SetItem?.Deleted == false)
            {
                dr["SetPropertyItemName"] = $"{sp.SetItem.Name},{sp.SetItem.GetType().Name}";
            }
            else
            {
            ...
            if (sp.WayPoint != null)
            {
                ...
                {
                    waystr = $"SERIAL,{sp.WayPoint.Serial}";
                }
            }
            dr["WayPoint"] = waystr;
            ...
            foreach (var i in World.Items.Values)
            {
                try
                {
                    if (i is XmlSpawner && (RespawnAll || i.Map == e.Mobile.Map) && i.Deleted == false)
                    {
                        ...
                        if (SpawnerPrefix == null || SpawnerPrefix.Length == 0 || i.Name?.StartsWith(SpawnerPrefix) == true)
                        {
                            ToRespawn.Add(i);
                            Count++;
                        }
    ...
    public static void XmlMake_OnCommand(CommandEventArgs e)
    {
        ...
        if (e.Arguments.Length > 0)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                if (e.Arguments.Length > 2)
                {
                    _ = new Spawner(10, 1, 1, 0, 2, e.Arguments[1])
                    {
                        Location = new Point3D(5400 + Utility.Random(700), 1090 + Utility.Random(180), 0),
                        Map = Map.Trammel
                    };
                }
                else
                if (e.Arguments.Length > 1)
                {
                    _ = new XmlSpawner(10, 1, 1, 0, 2, e.Arguments[1])
                    {
                        Location = new Point3D(5400 + Utility.Random(700), 1090 + Utility.Random(180), 0),
                        Map = Map.Trammel
                    };
    ...
    public static void XmlTrace_OnCommand(CommandEventArgs e)
    {
        ...
        var runningtime = Core.Now - _traceStartTime;
        ...
        foreach (var so in m_SpawnObjects)
        {
            for (var x = 0; x < so.SpawnedObjects.Count; x++)
            {
                ...
                if (o is Item item)
                {
                    ...
                    {
                        _ = so.SpawnedObjects.Remove(item);
                        x--;
                        removed = true;
                ...
                else if (o is Mobile mobile)
                {
                    ...
                    if (mobile.Deleted || despawned)
                    {
                        ...
                        _ = so.SpawnedObjects.Remove(mobile);
                        x--;
                        removed = true;
                        // if sequential spawning is active and the RestrictKillsToSubgroup flag is set, then check to see if
                        // the object is in the current subgroup before adding to the total
                    ...
                    else if (mobile is BaseCreature creature)
                    {
                        ...
                        if (creature.Controlled || creature.IsStabled || creature.Owners?.Count > 0)
                        {
                            _ = so.SpawnedObjects.Remove(mobile);
                            x--;
                            removed = true;
    ...
    {
        if (m_SpawnObjects?.Count > 0 && (m_proximityActivated || CanFreeSpawn) && TODInRange)
        {
    ...
    public bool Spawn(int index, bool smartspawn, int packrange, Point3D packcoord, bool ignoreloopprotection, byte loops)
    {
        ...
        if (TheSpawn != null)
        {
            ...
            if (BaseXmlSpawner.IsTypeOrItemKeyword(typeName))
            {
                ...
                var completedtypespawn = BaseXmlSpawner.SpawnTypeKeyword(this, TheSpawn, typeName, substitutedtypeName,
                    TriggerMob, Map, out var status_str, loops);
                ...
                try
                {
                    if (o is Mobile mob)
                    {
                        ...
                        if (mob is BaseCreature mobile)
                        {
                            ...
                            mobile.CurrentWayPoint = WayPoint;
                        ...
                        _ = BaseXmlSpawner.ApplyObjectStringProperties(this, substitutedtypeName, mob, TriggerMob, this, out var status_str);
    ...
    public Point3D GetPackCoord(int sgroup)
    {
        for (var j = 0; j < m_SpawnObjects.Count; j++)
        {
            ...
            if (so?.SubGroup == sgroup && so.SpawnedObjects.Count > 0 && so.PackRange >= 0)
            {
    ...
    public static void RefreshNextSpawnTime(SpawnObject so)
    {
        if (so == null)
        {
            return;
                ...
                if (wayargs[0] == "SERIAL")
                {
                    ...
                    if (wayargs.Length > 1)
                    {
                        ...
                        try
                        {
                            ...
                            var e = World.FindEntity((Serial)sernum);
    ...
    private static bool HasTileSurface(Map map, int X, int Y, int Z)
    {
        ...
        {
            if (staticTile.Z + staticTile.Height == Z)
            {
                return true;
            }
        }
    ...
    public bool CanFit(int x, int y, int z, int height, bool checkBlocksFit, bool checkMobiles, bool requireSurface, Mobile mob)
    {
        ...
        foreach (var staticTile in map.Tiles.GetStaticAndMultiTiles(x, y))
        {
            var id = TileData.ItemTable[staticTile.ID & TileData.MaxItemValue];
            surface = id.Surface;
            ...
            if ((surface || impassable) && staticTile.Z + id.CalcHeight > z && z + height > staticTile.Z)
            {
                return false;
            }

            if (surface && !impassable && z == staticTile.Z + id.CalcHeight)
            {
                hasSurface = true;
            }
        }
    ...
    public static bool HasRegionPoints(Region r) => r?.Area.Length > 0;

    public Rectangle2D SpawnerBounds => new(m_X, m_Y, m_Width + 1, m_Height + 1);

    private static void FindTileLocations(
    ...
    {
        Defrag(false);
        return m_SpawnObjects?[index]?.MaxCount ?? 0;
    }
    ...
    public void DeleteSpawnObject(Mobile from, string SpawnObjectName)
    {
        ...
        try
        {
            ...
            if (TheSpawn != null)
            {
                ...
                while (TheSpawn.SpawnedObjects?.Count > 0 && TheSpawn.SpawnedObjects.Count > TheSpawn.MaxCount)
                {
                ...
                if (m != null && (m.AccessLevel <= SmartSpawnAccessLevel || !m.Hidden))
                {
                    ...
                    if (m.Map != null && m.Map != Map.Internal)
                    {
                        ...
                        if (s != null && GlobalSectorTable[m.Map.MapID] != null)
                        {
                            ...
                            if (GlobalSectorTable[m.Map.MapID].TryGetValue(s, out var spawnerlist) && spawnerlist != null)
                            {
                                foreach (var spawner in spawnerlist)
                                {

                                    if (spawner?.Deleted == false && spawner.Running && spawner.SmartSpawning && spawner.IsInactivated)
                                    {
    ...
    private class SectorTimer : Timer
    {
        ...
        {
            // check the sectors
            if (m_Spawner?.Deleted == false && m_Spawner.Running && m_Spawner.IsInactivated)
            {
    ...
    public override void Deserialize(IGenericReader reader)
    {
        ...
        switch (version)
        {
            ...
            case 22:
                {
                    ...
                    TriggerMob = reader.ReadEntity<Mobile>();
                    goto case 21;
                }
            case 21:
                {
            ...
            case 10:
                {
                    SetItem = reader.ReadEntity<Item>();
                    goto case 9;
                }
            case 9:
                {
            ...
            case 0:
                {
                    ...
                    for (var i = 0; i < SpawnListSize; ++i)
                    {
                        ...
                        for (var x = 0; x < SpawnedCount; ++x)
                        {
                            ...
                            {
                                var e = World.FindEntity((Serial)(uint)serial);
    ...
    internal string GetSerializedObjectList2()
    {
        ...
        foreach (var so in m_SpawnObjects)
        {
            ...
            _ = sb.Append(
                $"{so.TypeName}:MX={so.ActualMaxCount}:SB={so.SubGroup}:RT={so.SequentialResetTime}:TO={so.SequentialResetTo}:KL={so.KillsNeeded}:RK={(so.RestrictKillsToSubgroup ? 1 : 0)}:CA={(so.ClearOnAdvance ? 1 : 0)}:DN={so.MinDelay}:DX={so.MaxDelay}:SP={so.SpawnsPerTick}:PR={so.PackRange}"
            );
        }
                    ...
                    if (SpawnObjectDetails.Length == 2)
                    {
                        ...
                        if (SpawnObjectDetails[0].Length > 0)
                        {
                            ...
                            if (SpawnObjectDetails[1].Length > 0)
                            {
                                ...
                                // Create the spawn object and store it in the array list
                                var so = new SpawnObject(SpawnObjectDetails[0], maxCount, subGroup, resetTime, resetTo, killsNeeded,
