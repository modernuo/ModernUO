...
public abstract class HarvestSystem
{
    ...
    {
        IncomingExtendedCommandPackets.RegisterExtended( 0x30, true, &TargetByResource );
    }

    public static void TargetByResource( NetState state, SpanReader reader )
    {
        if ( state.Mobile is not PlayerMobile { Alive: true } player )
        {
            return;
        }

        var serial = ( Serial )reader.ReadUInt32();
        int resourcetype = reader.ReadInt16();

        var tool = World.FindItem( serial );

        if ( tool is not BaseAxe baseAxe )
        {
            return;
        }

        var system = baseAxe.HarvestSystem;
        ...
        switch ( resourcetype )
        {
            case 0: // ore
                ...
                break;
            case 1: // sand
                if ( system is Mining harvestSystem )
                {
                    def = harvestSystem.Sand;
                }
            ...
            case 2: // wood
                ...
                {
                    def = lumberjacking.GetDefinition();
                }
        ...
        if ( def != null && FindValidTile( player, def, out var toHarvest ) )
        {
            system.StartHarvesting( player, tool, toHarvest );
            return;
        }

        system.OnBadHarvestTarget( player, tool, new LandTarget( new Point3D( 0, 0, 0 ), Map.Felucca ) );
    }
    ...
    private static bool FindValidTile( Mobile m, HarvestDefinition definition, out object toHarvest )
    {
        ...
        for ( var x = m.X - 1; x <= m.X + 1; x++ )
        {
            for ( var y = m.Y - 1; y <= m.Y + 1; y++ )
            {
                ...
                if ( definition.Validate( lt.ID, true ) )
                {
