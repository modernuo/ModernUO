...
public class ConfirmSignupGump : DynamicGump
{
    ...
    private ConfirmSignupGump(Mobile registrar, Tournament tourney, List<Mobile> players) : base(50, 50)
    {
    ...
    public static void DisplayTo(Mobile from, Mobile registrar, Tournament tourney, List<Mobile> players)
    {
        ...
        from.SendGump(new ConfirmSignupGump(registrar, tourney, players));
    }
    ...
    public override void OnResponse(NetState sender, in RelayInfo info)
    {
        ...
        if (info.ButtonID != 1 || !info.IsSwitched(1))
        {
            if (info.ButtonID > 1)
            {
                ...
                {
                    from.BeginTarget(12, false, TargetFlags.None, AddPlayer_OnTarget);
                    from.SendGump(this); // refresh-via-this
                }
            }
            ...
            case TournamentStage.Signup:
                {
                    ...
                    for (var i = 0; i < _players.Count; ++i)
                    {
                        ...
                        else if (_tournament.IsFactionRestricted && Faction.Find(mob) == null)
                        {
                            _registrar?.PrivateOverheadMessage(
                                MessageType.Regular,
                                0x35,
