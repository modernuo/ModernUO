...
public class TournamentBracketGump : DynamicGump
{
    ...
    private TournamentBracketGump(Tournament tourney, TourneyBracketGumpType type, List<object> list, int page, object obj)
        : base(50, 50)
    {
        _tournament = tourney;
        _type = type;
    ...
    )
    {
        ...
        from.SendGump(new TournamentBracketGump(tourney, type, list, page, obj), true);
    }
    ...
    public override void OnResponse(NetState sender, in RelayInfo info)
    {
        ...
        switch (type)
        {
            ...
            case 2:
                {
                    if (_type != TourneyBracketGumpType.Participant_List || index < 0 || index >= _list.Count)
                    {
                        return;
                    }
            ...
            case 3:
                {
                    if (_type != TourneyBracketGumpType.Round_List || index < 0 || index >= _list.Count)
                    {
                        return;
                    }
            ...
            case 4:
                {
                    if (_type != TourneyBracketGumpType.Participant_Info || _object is not TourneyParticipant part ||
                        index < 0 || index >= part.Players.Count)
                    {
                        return;
                    }
            ...
            case 5:
                {
                    if (_type != TourneyBracketGumpType.Round_Info || _object is not PyramidLevel level)
                    {
                        return;
                    }
            ...
            case 6:
                {
                    if (_type != TourneyBracketGumpType.Match_Info || _object is not TourneyMatch match || index < 0 ||
                        index >= match.Participants.Count)
                    {
                        return;
                    }
