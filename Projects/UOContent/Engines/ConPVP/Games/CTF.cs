            ...
            : base(60, 60)
        {
            ...
            if (section == null)
                for (int i = 0; i < entries.Count; ++i)
                {
                    ...
                    AddBorderedText(60, 65 + i * 75, 250, 20, $"{LadderGump.Rank(1 + i)}: {teamInfo.Name} Team", nameColor,
                        borderColor);
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (m_TeamInfo?.Game != null)
            {
                ...
                else
                {
                    ...
                    m_TeamInfo.Game.Alert("The {1} cookies have been stolen by {0} ({2}).", from.Name, ourTeam.Name,
                        useTeam.Name);
                ...
                if (teamInfo != null && teamInfo != victInfo)
                {
                    ...
                    if (playerInfo != null)
                    {
                        ...
                        for (int i = 0; i < Controller.TeamInfo.Length; ++i)
                        {
                            ...
                            {
                                if (ourFlagCarrier.Aggressors.Any(aggr =>
