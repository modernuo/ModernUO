...
{
    private DateTime _change = DateTime.Now;
    private DateTime _stomp = DateTime.Now;
    private DateTime _teleport = DateTime.Now;

    [Constructible]
    public DreadHorn() : base( AIType.AI_Spellweaving, FightMode.Closest, 10, 1 )
    {
        Name = "a Dread Horn";
        Body = 257;
        BaseSoundID = 0xA8;
        ...
        _change = DateTime.UtcNow;
        _stomp = DateTime.UtcNow;
        _teleport = DateTime.UtcNow;
    }
    ...
    public override void OnThink()
    {
        ...
        if ( Combatant != null )
        {
            if ( _change < DateTime.UtcNow && Utility.RandomDouble() < 0.1 )
            {
                ChangeOpponent();
            }

            if ( _stomp < DateTime.UtcNow && Utility.RandomDouble() < 0.1 )
            {
                HoofStomp();
            }

            if ( _teleport < DateTime.UtcNow && Utility.RandomDouble() < 0.1 )
            {
                Teleport();
            }
        }
    ...
    private void Teleport()
    {
        ...
        _teleport = DateTime.UtcNow + TimeSpan.FromSeconds( Utility.RandomMinMax( 40, 60 ) );
    }
    ...
    public void ChangeOpponent()
    {
        ...
        if ( random < 0.75 )
        {
            ...
            for ( var i = 0; i < Aggressors.Count && best == null; i++ )
            {
        ...
        if ( best != null )
        {
            ...
            _change = DateTime.UtcNow + TimeSpan.FromSeconds( Utility.RandomMinMax( 5, 10 ) );
        }
    }
    ...
    public void HoofStomp()
    {
        ...
        foreach ( var m in SpellHelper.AcquireIndirectTargets( this, Location, Map, StrikingRange ) /*.OfType<Mobile>()*/ )
        {
            if ( m.GetStatMod( "DreadHornStr" ) == null )
            {
                var percent = m.Skills.MagicResist.Value / 100;
            ...
            m.SendLocalizedMessage(
                1075081
            ); // *Dreadhorns eyes light up, his mouth almost a grin, as he slams one hoof to the ground!*
        }

        // earthquake
        PlaySound( 0x2F3 );
        _stomp = DateTime.UtcNow + TimeSpan.FromSeconds( Utility.RandomMinMax( 60, 80 ) );
    }

    public static bool IsUnderInfluence( Mobile m ) => m.GetStatMod( "DreadHornStr" ) != null;
