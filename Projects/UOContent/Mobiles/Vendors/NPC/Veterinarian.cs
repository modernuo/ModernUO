...
{
    private static readonly Dictionary<Mobile, Timer> _expireTable = new();
    ...
    public static BaseCreature[] GetDeadPets( Mobile from )
    {
        ...
        var eable = from.GetMobilesInRange( 12 );

        foreach ( var m in eable )
        {
            if ( m is BaseCreature { IsDeadBondedPet: true } bc && bc.ControlMaster == from && from.InLOS( bc ) )
            {
                pets.Add( bc );
            }
        }
    ...
    {
        var fee = ( int )( 100 + Math.Pow( 1.1041, bc.MinTameSkill ) );
    ...
    public static void ResetExpire( Mobile m )
    {
        ...
        if ( _expireTable.ContainsKey( m ) )
        {
            var t = _expireTable[m];
