...
namespace Server.Mobiles
{
    public class BaseEscortable : BaseCreature
    {
        ...
        protected override List<MLQuest> ConstructQuestList()
        {
            if (m_MLQuest == null)
            {
                ...
                for (int i = 0; i < list.Length; ++i)
                {
                    ...
                    {
                        Console.WriteLine("Warning: Escortable cannot be assigned quest type '{0}', it is not registered",
                            questType.Name);
                    }
        ...
        public virtual bool AcceptEscorter(Mobile m)
        {
            ...
            if (EscortTable.TryGetValue(m, out BaseEscortable escortable) && escortable?.Deleted == false &&
