...
namespace Server.Gumps
{
    ...
    public class AdminGump : Gump
    {
        ...
        public AdminGump(Mobile from, AdminGumpPage pageType, int listPage = 0, List<object> list = null,
            string notice = null,
                ...
                case AdminGumpPage.Accounts_Shared:
                    {
                        ...
                        if (m_List == null)
                        {
                            ...
                            m_List =
                                Utility.CastListContravariant<KeyValuePair<IPAddress, List<Account>>, object>(sharedAccounts);
                        }
                        else
                        {
                            ...
                            case 7:
                                {
                                    ...
                                    {
                                        results = Accounts.GetAccounts().Where(acct => acct.Username.ToLower().IndexOf(match) >= 0)
                                            .ToList();
                                        results.Sort(AccountComparer.Instance);
                                    }
                                    ...
                                    if (rads.Count > 0)
                                        from.SendGump(new WarningGump(1060635, 30720,
                                            string.Format(
                                                ...
                                                rads.Count, rads.Count == 1 ? "" : "s"), 0xFFC000, 420, 280,
                                    ...
                                    else
                                    {
                                        ...
                                        if (m_List != null && index >= 0 && index < m_List.Count)
                                        {
                                            if (m_List[index] is Account)
                                                ...
                                                from.SendGump(new AdminGump(from, AdminGumpPage.Accounts, 0,
                                                    Utility.CastListContravariant<Account, object>(kvp.Value), null,
                                                    new List<object>()));
                                        }
                                    }
