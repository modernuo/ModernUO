...
namespace Server.Gumps
{
    ...
    public class LootPackItemEditor : Gump
    {
        ...
        public LootPackItemEditor(Mobile mobile, object o) : base(GumpOffsetX, GumpOffsetY)
        {
        ...
        public LootPackItemEditor(Mobile mobile, object o, Stack<StackEntry> stack, List<object> list, int page) : base(
            GumpOffsetX,
            GumpOffsetY
        )
        {
        ...
        private void Initialize(int page)
        {
            ...
            AddBackground(0, 0, BackWidth, BorderSize + totalHeight + BorderSize + 150+ApplySize, BackGumpID);
            AddButton(BackWidth/3, BorderSize + totalHeight + BorderSize+150, 5204, 5205, 3);
            AddImageTiled(
                BorderSize,
                BorderSize,
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                case 3: // Apply
                    {
                        if (m_Object is LootItem itm)
                        {
                            var _pack = itm._savedpack;
                            _pack[itm.Idx] = itm;
                            from.SendGump(new LootPackCreationGump(itm.PackName, _pack));
                        }
                        if (m_Object is InitStats stat)
                        {
                            packLoader.AddToPack(stat.Name, stat);
                            from.SendMessage("Pack " + stat.Name + " is saved");
                        }
                        break;
                    }
                default:
                    {
                        ...
                        if (index >= 0 && index < m_List.Count)
                        {
                            ...
                            if (IsType(type, typeofBool))
                            {
                                from.SendGump(
                                    new SetListOptionGumpEx(
                                        prop,
                                        from,
                                        m_Object,
                                        m_Stack,
                            ...
                            {
                                from.SendGump(new SetGumpEX(prop, from, m_Object, m_Stack, m_Page, m_List, 2));
                            }
